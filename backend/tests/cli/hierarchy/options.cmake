...
run_error("unknown analysis" "${map}" --analysis invalid)
...
endforeach()
run_error("MAP input path is required" --analysis hierarchy-rd)
run_error("--event-limit.*--export-events" ${base} --event-limit 0)
run_error("hierarchy-rd" "${map}" --telemetry "${telemetry_file}")
run_error("hierarchy-rd" "${map}" --analysis mapping --elf "${YARDA_ELF}"
    --cache "${YARDA_CACHE}" --export-events "${events_file}")
run_error("--elf.*required" "${map}" --analysis mapping --cache "${YARDA_CACHE}")
...
endforeach()
run_error("hierarchy-rd" "${map}" --max-source-accesses 0)
...
run_ok("${map}" --elf "${YARDA_ELF}" --cache "${YARDA_CACHE}"
    --export "${YARDA_WORK_DIR}/implicit.json")
run_ok("${map}" --analysis mapping --elf "${YARDA_ELF}" --cache "${YARDA_CACHE}"
