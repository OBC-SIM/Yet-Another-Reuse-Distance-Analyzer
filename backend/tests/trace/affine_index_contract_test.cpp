...
TEST_P(AffineIndexContract, MatchesSourceElementOrderAndLinkedByteAddresses)
{
  ...
  EXPECT_EQ(tasks,
            (std::vector<std::string>{
              "offset", "stride", "descending", "constant", "dimensions",
              "caller", "fields", "shadowed", "capture", "unsigned_caller",
              "unsigned_forward", "unsigned_boundary"}));
  uint64_t count = 0;
  for (const auto & [task, expected] : elements)
  {
    ...
    const std::string object = task == "unsigned_boundary" ? "global::unsigned_"
                                                             "matrix"
                               : task == "dimensions"      ? "global::matrix"
                               : task == "fields"          ? "global::records"
                                                           : "global::a";
    for (std::size_t i = 0; i < expected.size(); ++i)
    {
      SCOPED_TRACE(i);
      const auto offset =
        task == "fields" ? 8 * expected[i] + 4 : 4 * expected[i];
