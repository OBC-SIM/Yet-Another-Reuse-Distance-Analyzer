...
std::optional<ByteAccess> AccessLayoutResolver::prepare(
  const Json & node, const std::vector<std::int64_t> & indices,
  PreparedLayout & plan) const
{
  plan = {};
  if (!has_field_path(node)) return prepare_legacy_access(node, indices, &plan);
  return prepare_structured_access(node, indices, objects_, structs_, plan);
}
