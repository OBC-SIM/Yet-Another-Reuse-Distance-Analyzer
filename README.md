...
| `test_gt_cache.py` | affine index extent 기반 ground-truth 주소 선형화 |
| `test_stability.py` | 2D/3D stable RD holdout 검증 |
| `test_plot_timing.py` | timing plot의 ms 단위 변환 및 집계 단위 보존 |
...
| `verify_<stem>_timing_blocks.png`  | 블록별 unroll 시간 vs. 예측 시간 (ms) |
| `verify_<stem>_timing_program.png` | 프로그램 전체 합산 타이밍 (ms) |
...
| polybench_correlation.c | ✅ MATCH |
| polybench_jacobi.c | ✅ MATCH |

`unstable` 브랜치는 sample마다 volatile RD group count가 달라지는 3D sparse/tail family를 `volatile3d_variable.py`의 group-family extrapolation으로 처리합니다. `polybench_jacobi.c`의 affine stencil index는 `gt_cache.py`에서 배열별 모든 access index의 min/max extent를 반영해 ground-truth 주소 선형화를 보정합니다.

남은 일반화 과제는 variable 3D extrapolation key 선택입니다. 현재는 target shape의 `max/min > 4` 여부로 max-axis/min-axis key를 고르는 휴리스틱을 쓰며, 추후 holdout error 기반 key selection으로 교체하는 것이 목표입니다.
...
├── predictor3d.py        # 3D stable/volatile RD 예측 오케스트레이션
├── series.py             # quadratic / linear / constant-third-difference series extrapolation
...
├── plot_timing.py        # 실행시간 비교 차트 (ms 단위 표시)
├── stability.py          # stable RD 후보 검증 (2D/3D holdout validation)
├── volatile.py           # 공용 volatile helper + 3D diagonal/rectangular RD 예측
├── volatile2d.py         # 2D Volatile RD 예측
├── volatile3d.py         # 3D diagonal/rectangular Volatile RD 예측
├── volatile3d_variable.py# sample별 group count가 달라지는 3D Volatile RD 예측
...
| Volatile RD 예측 (2D rectangular, 3D diagonal/rectangular/variable group) | ✅ |
| 3D stable RD holdout validation (`stability.py`) | ✅ |
| affine index extent 기반 GT cache 주소 선형화 | ✅ |
...
| 실행시간 비교 시각화 (`plot_timing.py`, verify `--plot`, ms 단위) | ✅ |
| `--mode unroll`로 실제 LRU 시뮬 결과 출력 (`main.py`) | ✅ |
| `polybench_correlation.c` 마지막 3D sparse/tail family | ✅ |
| `polybench_jacobi.c` sequential 2D stencil nests | ✅ |
| variable 3D extrapolation key의 holdout 기반 선택 | 🔲 미구현 |
| 4중 루프 이상 지원 | 🔲 미구현 |
