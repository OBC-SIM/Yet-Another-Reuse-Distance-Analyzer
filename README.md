...
**C++ 프론트엔드** ([`frontend/`](https://github.com/OBC-SIM/Access-Pattern-Extractor), [Access-Pattern-Extractor](https://github.com/OBC-SIM/Access-Pattern-Extractor) 서브모듈)가 LLVM Pass로 IR을 분석해 루프 구조, 시작 값, 배열 접근 패턴을 JSON으로 출력합니다. **Python 백엔드** (`backend/`)는 이 JSON을 받아 소규모 LRU 시뮬레이션으로 Dilation Equation 계수를 도출하고, 실제 바운드에서의 RDH를 정적으로 예측합니다.
...
# cache-line 단위 unroll RDH (spatial locality reference model)
python backend/main.py tasks/test_1d.c --mode unroll --granularity cache-line
```
...
| `GetIndexVars` | 전역/파라미터 배열 GEP chain의 다차원 index 보존 |
...
기본 RD/CA score는 element-level temporal reuse 기준입니다.
`main.py --mode unroll --granularity cache-line`을 사용하면 추론 가능한
`shape`/`elem_size` metadata를 바탕으로 cache-line 단위 RDH를 계산합니다.
정적 predictor의 cache-line 지원은 아직 별도 과제로 남아 있습니다.
...
```
frontend/                 # C++ LLVM Pass 서브모듈 (Access-Pattern-Extractor)
├── include/
...
│   └── IrHelpers.hpp         # IR 쿼리 헬퍼 선언 (NameMap, getBaseName, getIndexVars 등)
├── src/
...
│   └── LoopAnalysisPass.cpp  # LLVM Pass: buildRootStatements → LAT JSON 출력
├── tests/
│   ├── Statement_test.cpp    # AST / JSON 직렬화 단위 테스트
│   ├── IrHelpers_test.cpp    # IR 헬퍼 단위 테스트 (LLVM IR 직접 생성)
│   ├── IrHelpersGep_test.cpp # 전역/파라미터 배열 GEP index 보존 테스트
│   └── ArrayMetadata_test.cpp# shape / elem_size metadata 테스트
└── CMakeLists.txt
backend/
├── block_trace.py        # LAT body 순서를 보존하는 actual trace / block trace helper
├── ca_metrics.py         # mean RD / CA score 계산 및 출력 포맷
...
├── test_matmul_params.c  # 함수 파라미터 2D 배열 GEP 회귀 케이스
├── test_mixed_global_param.c # 전역/파라미터 2D 배열 혼합 접근
...
| 배열 `shape` / `elem_size` metadata 출력 | ✅ |
| 함수 파라미터 2D 배열 GEP chain의 outer index 보존 | ✅ |
...
| mean RD / CA score 공통 계산 (`ca_metrics.py`) | ✅ |
...
| cache-line 기반 unroll/reference RDH | ✅ |
| cache-line 기반 static predictor | 🔲 미구현 |
